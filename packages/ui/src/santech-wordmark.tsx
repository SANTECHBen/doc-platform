...
// SANTECH wordmark — the platform attribution that appears on:
//
//   • The PWA topbar's "Powered by SANTECH" cluster (right side of the
//     asset hub header on every tab).
//   • The admin sign-in card footer ("Powered by SANTECH" line above
//     the SANTECH-internal version line).
//
// Both consumers historically inlined the same SVG paths verbatim. The
// component centralizes that and accepts a className so consumers
// control size + color via their own CSS classes (.app-topbar-powered-by-mark
// in the PWA, .signin-santech-mark in admin). Width is `auto` so the
// aspect ratio is preserved when the consumer sets height.
//
// Color split:
//   • "SAN" — uses currentColor so it inherits the theme ink (works in
//     both light and dark).
//   • "TECH" — hard-coded to the SANTECH brand red (#e11d24) regardless
//     of theme; the brand color is the brand color.

export interface SantechWordmarkProps {
  /** Tailwind/utility classes for size + base color. The component
   *  itself only sets the SVG path geometry; sizing and theme color
   *  are the caller's job. */
  className?: string;
}

export function SantechWordmark({ className }: SantechWordmarkProps) {
  return (
    <svg
