...
export async function registerFieldProcedureRoutes(app: FastifyInstance) {
  ...
  app.post<{
    ...
    async (request, reply) => {
      ...
      // Synthesize a bodyMarkdown from the captured steps + substeps so the
      // extraction pipeline has something to chunk and embed. Without this,
      // processMarkdownDocument sees empty bodyMarkdown and marks the doc
      // not_applicable — every field-authored procedure becomes invisible
      // to the troubleshooter ("retrieved chunks contain no procedure for
      // belt removal" even though the tech literally just authored one).
      try {
        const synthesized = await synthesizeFieldDocMarkdown(
          db,
          ctx.document.id,
          body.title,
        );
        if (synthesized.trim().length > 0) {
          await db
            ...
            .where(eq(schema.documents.id, ctx.document.id));
          // Fire-and-forget — extraction can take a few seconds because of
          // the embedding round-trip. The HTTP response returns immediately;
          // chunks/embeddings show up in the chat retriever shortly after.
          triggerExtraction(app, ctx.document.id);
        }
      } catch (err) {
        // Non-fatal: leaves the doc visible in the Documents tab but absent
        // from chat retrieval until a re-trigger. Worth surfacing to logs.
        request.log.error(
          { err, documentId: ctx.document.id },
          'failed to synthesize/extract field-procedure markdown',
        );
      }
...
// Build a flat markdown body from the captured steps + substeps of a
// field-authored procedure document. The chat retriever runs on
// document_chunks, which the AI pipeline produces by chunking
// documents.bodyMarkdown — but field-authored procedures store their
// content in procedure_steps, not in the documents row, so without this
// synthesis the doc would have no chunks and be invisible to chat.
//
// Output shape mirrors what a hand-written procedure markdown would look
// like, so retrieval scoring is comparable across both sources.
async function synthesizeFieldDocMarkdown(
  db: Database,
  documentId: string,
  title: string,
): Promise<string> {
    ...
    if (subs.length > 0) {
      ...
      subs.forEach((ss, j) => {
        ...
        if (ss.bodyMarkdown && ss.bodyMarkdown.trim().length > 0) {
          // Indent the substep body so it nests under the list item.
