...
export function NavSidebar({
  ...
  return (
    <aside className="sticky top-0 flex h-screen w-60 shrink-0 flex-col border-r border-white/[0.04] bg-surface-sidebar text-white/[0.85]">
            ...
            <li key={group.id}>
              {group.label && (
                <div className="relative flex items-center justify-between px-3 pb-1 pt-1">
                  <span className="font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-white/[0.38]">
                    {group.label}
                  </span>
                  {group.info && (
                    <>
                      <button
                        ...
                        data-open={openInfo === group.id ? 'true' : 'false'}
                        className="grid h-4 w-4 place-items-center rounded-full text-white/40 transition data-[open=true]:bg-brand/[0.18] data-[open=true]:text-brand"
                      >
                      ...
                      {openInfo === group.id && (
                        <div
                          ...
                          className="absolute left-full top-0 z-50 ml-2 w-64 rounded-md border border-white/[0.08] bg-[#1a1d24] p-3 text-xs leading-relaxed text-white/[0.85] shadow-xl"
                        >
                          <div className="mb-1 font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-brand">
                            {group.label}
                          </div>
                          <p>{group.info}</p>
                        </div>
                  ...
                  return (
                    <li key={n.href}>
                      <Link
                        ...
                        className="group relative flex items-center gap-2.5 rounded px-3 py-2 text-sm font-medium text-white/[0.65] transition data-[active=true]:bg-brand/[0.18] data-[active=true]:text-white"
                      >
                        {active && (
                          <span
                            className="absolute left-[-8px] top-2 bottom-2 w-[3px] rounded-r bg-brand"
                            aria-hidden
                            style={{ boxShadow: '0 0 10px rgb(var(--brand) / 0.4)' }}
                          />
                        )}
                        <Icon
                          size={16}
                          strokeWidth={active ? 2 : 1.75}
                          className={active ? 'text-brand' : 'text-white/60'}
                        />
                        <span>{n.label}</span>
                      </Link>
                    </li>
      ...
      <footer className="flex flex-col gap-2 border-t border-white/[0.06] px-5 py-3.5">
        {userMenu}
        <div className="flex items-center justify-between">
          <Link
            href="/design-system"
            className="inline-flex items-center gap-1.5 text-[10.5px] font-medium uppercase tracking-[0.08em] text-white/40 transition hover:text-white/70"
